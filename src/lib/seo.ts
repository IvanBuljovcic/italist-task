...
const SITE_DESCRIPTION = "Discover premium fashion products from top brands. Shop clothing, accessories, and bags with fast shipping.";
...
export function generateProductPageMetadata(params: SEOParams): Metadata {
	...
	if (sizes && sizes.length > 0) {
		...
		title = search 
			? `"${search}" in ${sizeText} | ${SITE_NAME}`
			: `${sizeText} Products | ${SITE_NAME}`;
